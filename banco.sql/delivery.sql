...
-- =====================================================
-- TRIGGERS PARA VALIDAÇÕES E NOTIFICAÇÕES
-- =====================================================

DELIMITER //

-- =====================================================
-- TRIGGER 1: Validar se entregador pode assumir pedido
-- =====================================================
CREATE TRIGGER trg_validar_entregador_pedido
BEFORE UPDATE ON pedido
FOR EACH ROW
BEGIN
    DECLARE v_status_entregador VARCHAR(20);
    DECLARE v_entregador_ativo BOOLEAN;
    DECLARE v_mensagem VARCHAR(255);
    
    -- Verificar se está tentando atribuir um entregador
    IF NEW.entregador_id IS NOT NULL AND (OLD.entregador_id IS NULL OR OLD.entregador_id != NEW.entregador_id) THEN
        
        -- Verificar se entregador existe e está ativo
        SELECT status, u.ativo INTO v_status_entregador, v_entregador_ativo
        FROM entregador e
        JOIN usuario u ON e.id = u.id
        WHERE e.id = NEW.entregador_id;
        
        -- Validações
        IF v_entregador_ativo = FALSE THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Entregador inativo no sistema';
        END IF;
        
        IF v_status_entregador != 'disponivel' THEN
            SET v_mensagem = CONCAT('Entregador não está disponível. Status atual: ', v_status_entregador);
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
        END IF;
        
        -- Verificar se o entregador já não está com muitos pedidos em andamento
        IF (SELECT COUNT(*) FROM pedido 
            WHERE entregador_id = NEW.entregador_id 
            AND status IN ('em_entrega', 'aguardando_entregador')) >= 3 THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Entregador já possui 3 pedidos em andamento';
        END IF;
        
        -- Registrar log de atribuição
        INSERT INTO log_atividade (usuario_id, acao, descricao, ip_address)
        VALUES (NEW.entregador_id, 'atribuido_pedido', 
                CONCAT('Entregador atribuído ao pedido ', NEW.id), NULL);
    END IF;
END//

-- =====================================================
-- TRIGGER 2: Validar status do pedido antes de atualizar
-- =====================================================
...
BEGIN
    ...
    IF OLD.status != NEW.status THEN
        ...
        CASE OLD.status
            WHEN 'aguardando_pagamento' THEN
                IF NEW.status NOT IN ('pagamento_aprovado', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "aguardando_pagamento" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'pagamento_aprovado' THEN
                IF NEW.status NOT IN ('em_preparo', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "pagamento_aprovado" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'em_preparo' THEN
                IF NEW.status NOT IN ('pronto', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "em_preparo" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'pronto' THEN
                IF NEW.status NOT IN ('aguardando_entregador', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "pronto" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'aguardando_entregador' THEN
                IF NEW.status NOT IN ('em_entrega', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "aguardando_entregador" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'em_entrega' THEN
                IF NEW.status NOT IN ('entregue', 'cancelado') THEN
                    SET v_mensagem = CONCAT('Não é possível mudar de "em_entrega" para "', NEW.status, '"');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
                
            WHEN 'cancelamento_solicitado' THEN
                IF NEW.status NOT IN ('cancelado') THEN
                    SET v_mensagem = CONCAT('Pedido com cancelamento solicitado só pode ser cancelado');
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_mensagem;
                END IF;
            ...
            WHEN 'cancelado' THEN
                IF NEW.status != 'cancelado' THEN
                    SIGNAL SQLSTATE '45000' 
                    SET MESSAGE_TEXT = 'Pedido cancelado não pode ter status alterado';
                END IF;
                
            WHEN 'entregue' THEN
                IF NEW.status != 'entregue' THEN
                    SIGNAL SQLSTATE '45000' 
                    SET MESSAGE_TEXT = 'Pedido entregue não pode ter status alterado';
                END IF;
        END CASE;
    END IF;
END//
...
-- =====================================================
-- TRIGGER 3: Validar dados do entregador antes de inserir/atualizar
-- =====================================================
...
BEGIN
    -- Validar CNH
    IF NEW.cnh IS NULL OR LENGTH(NEW.cnh) < 9 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'CNH inválida ou não informada';
    END IF;
    
    -- Validar veículo
    IF NEW.tipo_veiculo = 'moto' AND NEW.placa_veiculo IS NULL THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Moto deve ter placa informada';
    END IF;
    
    IF NEW.tipo_veiculo = 'carro' AND NEW.placa_veiculo IS NULL THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Carro deve ter placa informada';
    END IF;
    
    -- Validar modelo do veículo
    IF NEW.tipo_veiculo IN ('moto', 'carro') AND NEW.modelo_veiculo IS NULL THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Modelo do veículo deve ser informado';
    END IF;
END//

CREATE TRIGGER trg_validar_atualizacao_entregador
BEFORE UPDATE ON entregador
FOR EACH ROW
BEGIN
    -- Validar status do entregador
    IF NEW.status = 'ocupado' AND OLD.status = 'disponivel' THEN
        -- Verificar se realmente tem pedido atribuído
        IF (SELECT COUNT(*) FROM pedido 
            WHERE entregador_id = NEW.id 
            AND status IN ('em_entrega', 'aguardando_entregador')) = 0 THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Entregador não pode ficar ocupado sem pedidos atribuídos';
        END IF;
    END IF;
    
    -- Validar se pode ficar offline
    IF NEW.status = 'offline' AND OLD.status != 'offline' THEN
        IF (SELECT COUNT(*) FROM pedido 
            WHERE entregador_id = NEW.id 
            AND status IN ('em_entrega', 'aguardando_entregador')) > 0 THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Entregador com pedidos em andamento não pode ficar offline';
        END IF;
    END IF;
END//

-- =====================================================
-- TRIGGER 4: Validar disponibilidade do produto antes de adicionar ao pedido
-- =====================================================
CREATE TRIGGER trg_validar_produto_pedido
BEFORE INSERT ON item_pedido
FOR EACH ROW
BEGIN
    DECLARE v_disponivel BOOLEAN;
    DECLARE v_restaurante_pedido INT;
    DECLARE v_restaurante_produto INT;
    
    -- Verificar se produto está disponível
    SELECT disponivel, restaurante_id INTO v_disponivel, v_restaurante_produto
    FROM produto WHERE id = NEW.produto_id;
    
    IF v_disponivel = FALSE THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Produto não está disponível no momento';
    END IF;
    
    -- Verificar se o produto pertence ao mesmo restaurante do pedido
    SELECT restaurante_id INTO v_restaurante_pedido
    FROM pedido WHERE id = NEW.pedido_id;
    
    IF v_restaurante_produto != v_restaurante_pedido THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Produto não pertence ao restaurante do pedido';
    END IF;
    ...
    IF NEW.quantidade > 50 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Quantidade máxima por item é 50 unidades';
    END IF;
END//

-- =====================================================
-- TRIGGER 5: Atualizar avaliação média do entregador após entrega
-- =====================================================
CREATE TRIGGER trg_atualizar_avaliacao_entregador
AFTER UPDATE ON pedido
FOR EACH ROW
BEGIN
    DECLARE v_nova_media DECIMAL(3,2);
    
    -- Quando o pedido é entregue e tem avaliação
    IF NEW.status = 'entregue' AND NEW.avaliacao IS NOT NULL AND OLD.avaliacao IS NULL THEN
        
        -- Calcular nova média
        SELECT AVG(avaliacao) INTO v_nova_media
        FROM pedido
        WHERE entregador_id = NEW.entregador_id 
        AND status = 'entregue' 
        ...
        UPDATE entregador 
        SET avaliacao_media = v_nova_media,
            total_entregas = total_entregas + 1
        WHERE id = NEW.entregador_id;
        
        -- Registrar log
        INSERT INTO log_atividade (usuario_id, acao, descricao)
        VALUES (NEW.entregador_id, 'avaliacao_recebida', 
                CONCAT('Avaliação ', NEW.avaliacao, ' para pedido ', NEW.id));
    END IF;
END//

-- =====================================================
-- TRIGGER 6: Validar endereço antes de criar pedido
-- =====================================================
CREATE TRIGGER trg_validar_endereco_pedido
BEFORE INSERT ON pedido
FOR EACH ROW
BEGIN
    DECLARE v_endereco_completo INT;
    DECLARE v_cep_valido INT;
    
    -- Verificar se endereço existe
    SELECT COUNT(*) INTO v_endereco_completo
    FROM endereco 
    WHERE id = NEW.endereco_entrega_id 
    AND logradouro IS NOT NULL 
    AND cep IS NOT NULL;
    
    IF v_endereco_completo = 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Endereço de entrega inválido ou incompleto';
    END IF;
    
    -- Verificar se endereço pertence ao cliente
    IF (SELECT usuario_id FROM endereco WHERE id = NEW.endereco_entrega_id) != NEW.cliente_id THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Endereço não pertence ao cliente';
    END IF;
    
    -- Validar CEP (formato básico)
    SET v_cep_valido = (SELECT cep REGEXP '^[0-9]{5}-[0-9]{3}$' FROM endereco WHERE id = NEW.endereco_entrega_id);
    
    IF v_cep_valido = 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'CEP em formato inválido. Use XXXXX-XXX';
    END IF;
END//

-- =====================================================
-- TRIGGER 7: Validar horário de funcionamento do restaurante
-- =====================================================
CREATE TRIGGER trg_validar_horario_restaurante
BEFORE INSERT ON pedido
FOR EACH ROW
BEGIN
    DECLARE v_horario_abertura TIME;
    DECLARE v_horario_fechamento TIME;
    DECLARE v_hora_atual TIME;
    DECLARE v_dia_semana INT;
    DECLARE v_funciona BOOLEAN DEFAULT FALSE;
    
    -- Obter horários do restaurante
    SELECT horario_abertura, horario_fechamento, dias_funcionamento 
    INTO v_horario_abertura, v_horario_fechamento, v_dias_funcionamento
    FROM restaurante 
    WHERE id = NEW.restaurante_id;
    
    SET v_hora_atual = TIME(NOW());
    SET v_dia_semana = DAYOFWEEK(NOW());
    
    -- Verificar dia de funcionamento
    IF v_dias_funcionamento = 'todos' THEN
        SET v_funciona = TRUE;
    ELSEIF v_dias_funcionamento = 'segunda-sexta' AND v_dia_semana BETWEEN 2 AND 6 THEN
        SET v_funciona = TRUE;
    ELSEIF v_dias_funcionamento = 'fim-de-semana' AND v_dia_semana IN (1, 7) THEN
        SET v_funciona = TRUE;
    END IF;
    
    IF NOT v_funciona THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Restaurante não funciona neste dia';
    END IF;
    
    -- Verificar horário
    IF v_hora_atual < v_horario_abertura OR v_hora_atual > v_horario_fechamento THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Restaurante está fechado no momento';
    END IF;
END//

-- =====================================================
-- TRIGGER 8: Validar dados do pagamento
-- =====================================================
...
BEGIN
    ...
    -- Validar método de pagamento com dados fornecidos
    IF NEW.metodo IN ('cartao_credito', 'cartao_debito') AND NEW.dados_pagamento_cript IS NULL THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Dados do cartão são obrigatórios';
    END IF;
    
    IF NEW.metodo = 'pix' AND NEW.codigo_transacao IS NULL THEN
        -- Em produção, seria opcional, mas podemos sugerir
        -- SIGNAL SQLSTATE '45000' 
        -- SET MESSAGE_TEXT = 'Código da transação PIX é obrigatório';
    END IF;
END//

-- =====================================================
-- TRIGGER 9: Notificar quando pedido estiver pronto
-- =====================================================
CREATE TRIGGER trg_notificar_pedido_pronto
AFTER UPDATE ON pedido
FOR EACH ROW
BEGIN
    DECLARE v_tempo_espera INT;
    
    -- Quando o pedido fica pronto
    IF NEW.status = 'pronto' AND OLD.status != 'pronto' THEN
        
        -- Calcular tempo de preparo
        SET v_tempo_espera = TIMESTAMPDIFF(MINUTE, NEW.data_pedido, NOW());
        
        -- Registrar log de notificação
        INSERT INTO log_atividade (usuario_id, acao, descricao)
        VALUES (NEW.cliente_id, 'pedido_pronto', 
                CONCAT('Pedido ', NEW.id, ' está pronto. Tempo de preparo: ', v_tempo_espera, ' minutos'));
        
        -- Se houver entregador disponível, atualizar status automaticamente
        IF EXISTS (SELECT 1 FROM entregador WHERE status = 'disponivel' LIMIT 1) THEN
            -- Aqui poderia disparar uma notificação para o entregador
            INSERT INTO log_atividade (usuario_id, acao, descricao)
            VALUES (NULL, 'entregador_disponivel', 
                    CONCAT('Pedido ', NEW.id, ' aguardando entregador'));
        END IF;
    END IF;
END//

-- =====================================================
-- TRIGGER 10: Validar CPF e CNPJ únicos por tipo de usuário
-- =====================================================
...
BEGIN
    ...
    -- Verificar se CPF já está cadastrado em outro cliente
    SELECT COUNT(*) INTO v_cpf_existe
    FROM usuario u
    JOIN cliente c ON u.id = c.id
    WHERE u.cpf = (SELECT cpf FROM usuario WHERE id = NEW.id)
    AND u.id != NEW.id;
...
CREATE TRIGGER trg_validar_cnpj_restaurante
BEFORE INSERT ON restaurante
FOR EACH ROW
BEGIN
    -- Validar formato do CNPJ básico
    IF NEW.cnpj NOT REGEXP '^[0-9]{2}\.[0-9]{3}\.[0-9]{3}/[0-9]{4}-[0-9]{2}$' 
       AND NEW.cnpj NOT REGEXP '^[0-9]{14}$' THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'CNPJ em formato inválido. Use XX.XXX.XXX/XXXX-XX ou 14 dígitos';
    END IF;
END//

-- =====================================================
-- TRIGGER 11: Validar limite de itens por pedido
-- =====================================================
CREATE TRIGGER trg_validar_limite_itens_pedido
BEFORE INSERT ON item_pedido
FOR EACH ROW
BEGIN
    ...
    -- Somar com o novo item
    SET v_total_itens = IFNULL(v_total_itens, 0) + NEW.quantidade;
    SET v_valor_total = IFNULL(v_valor_total, 0) + (NEW.quantidade * NEW.preco_unitario);
    
    -- Limites
    IF v_total_itens > 30 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Limite máximo de 30 itens por pedido excedido';
    END IF;
    
    IF v_valor_total > 5000 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Valor máximo por pedido é R$ 5.000,00';
    END IF;
END//

-- =====================================================
-- TRIGGER 12: Atualizar status do entregador automaticamente
-- =====================================================
CREATE TRIGGER trg_atualizar_status_entregador_pedido
AFTER UPDATE ON pedido
FOR EACH ROW
BEGIN
    -- Quando um pedido é atribuído, atualizar status do entregador
    ...
    -- Quando todos os pedidos do entregador são entregues, voltar para disponível
    IF OLD.status IN ('em_entrega', 'aguardando_entregador') 
       AND NEW.status IN ('entregue', 'cancelado') THEN
        
        IF NOT EXISTS (SELECT 1 FROM pedido 
                       WHERE entregador_id = NEW.entregador_id 
                       AND status IN ('em_entrega', 'aguardando_entregador')) THEN
            UPDATE entregador 
...
-- =====================================================
-- CONSULTAS PARA VERIFICAR TRIGGERS
-- =====================================================

-- Listar todos os triggers criados
SHOW TRIGGERS;

-- Verificar estrutura dos triggers
SELECT 
    TRIGGER_NAME,
    EVENT_MANIPULATION,
    EVENT_OBJECT_TABLE,
...
WHERE TRIGGER_SCHEMA = 'sistema_delivery';
...
/*
-- Teste Trigger 1: Tentar atribuir entregador indisponível
UPDATE pedido SET entregador_id = 11 WHERE id = 3;
-- Deve funcionar se entregador estiver disponível

-- Teste Trigger 2: Tentar mudança de status inválida
UPDATE pedido SET status = 'em_entrega' WHERE id = 4 AND status = 'em_preparo';
-- Deve dar erro porque não pode pular status

-- Teste Trigger 3: Tentar cadastrar entregador sem CNH
INSERT INTO entregador (id, cnh, tipo_veiculo) 
VALUES (16, NULL, 'moto');
-- Deve dar erro

-- Teste Trigger 4: Adicionar produto de outro restaurante ao pedido
INSERT INTO item_pedido (pedido_id, produto_id, quantidade, preco_unitario)
VALUES (1, 4, 1, 34.90);
-- Deve dar erro

-- Teste Trigger 5: Avaliar entregador
UPDATE pedido SET status = 'entregue', avaliacao = 5 WHERE id = 1;
-- Deve atualizar média do entregador

-- Teste Trigger 6: Tentar criar pedido com endereço de outro cliente
INSERT INTO pedido (cliente_id, restaurante_id, endereco_entrega_id, valor_subtotal, valor_taxa_entrega, valor_total)
VALUES (7, 16, 1, 0, 8.50, 8.50);
-- Deve dar erro

-- Teste Trigger 7: Tentar fazer pedido fora do horário
-- Assume que restaurante 16 fecha às 23:00
INSERT INTO pedido (cliente_id, restaurante_id, endereco_entrega_id, valor_subtotal, valor_taxa_entrega, valor_total)
VALUES (6, 16, 1, 0, 8.50, 8.50);
-- Se for após 23:00, deve dar erro
*/